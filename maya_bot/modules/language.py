...
from maya_bot import decorator, logger, mongodb, redis
...
f = open('maya_bot/modules/langs/en.json', "r")
lang = ujson.load(f)
exec("LANGUAGES[\"" + lang['language_info']['code'] + "\"] = lang")
LANGS += tuple([lang['language_info']['code']])

for filename in os.listdir('maya_bot/modules/langs'):
    if filename == 'en.json':
        continue
    logger.debug("Loading language file " + filename)
    f = open('maya_bot/modules/langs/' + filename, "r")
    lang = ujson.load(f)
